...
public class TestScenarioWithInterceptorsOnClientSide extends Scenario {
	...
	public void constructAndRun() throws Exception {
		...
		Function<ChatMsgContent, Boolean> conditionForInterceptingI1OnC1 = msg -> msg.getSender() == 0;
		Function<ChatMsgContent, Boolean> conditionForExecutingI1OnC1 = msg -> true;
		Consumer<ChatMsgContent> treatmentI1OnC1 = msg -> chat.client.algorithms.chat.Action.CHAT_MESSAGE
				.execute(c1, new ChatMsgContent(msg.getSender(), msg.getSeqNumber(),
						msg.getContent() + ", intercepted at client c1 by i1"));
		ClientInterceptors.addAnInterceptor("i1", c1, conditionForInterceptingI1OnC1, conditionForExecutingI1OnC1,
				treatmentI1OnC1);
		Function<ChatMsgContent, Boolean> conditionForInterceptingI2OnC2 = msg -> msg.getSender() == 0;
		Function<ChatMsgContent, Boolean> conditionForExecutingI2OnC2 = msg -> true;
		Consumer<ChatMsgContent> treatmentI2OnC2 = msg -> chat.client.algorithms.chat.Action.CHAT_MESSAGE
				.execute(c2, new ChatMsgContent(msg.getSender(), msg.getSeqNumber(),
						msg.getContent() + ", intercepted at client c2 by i2"));
		ClientInterceptors.addAnInterceptor("i2", c2, conditionForInterceptingI2OnC2, conditionForExecutingI2OnC2,
				treatmentI2OnC2);
		Function<ChatMsgContent, Boolean> conditionForInterceptingI3OnC1 = msg -> msg.getSender() == 200;
		// next lambda for emulating situation in which message from client 200 to
		// client c1 never received
		Function<ChatMsgContent, Boolean> conditionForExecutingI3OnC1 = msg -> false;
		Consumer<ChatMsgContent> treatmentI3OnC1 = msg -> chat.client.algorithms.chat.Action.CHAT_MESSAGE
				.execute(c1, new ChatMsgContent(msg.getSender(), msg.getSeqNumber(),
						msg.getContent() + ", intercepted at client c1 by i3"));
		ClientInterceptors.addAnInterceptor("i3", c1, conditionForInterceptingI3OnC1, conditionForExecutingI3OnC1,
				treatmentI3OnC1);
		Consumer<ChatMsgContent> treatmentI4OnC1 = msg -> chat.client.algorithms.chat.Action.CHAT_MESSAGE
				.execute(c1, new ChatMsgContent(msg.getSender(), msg.getSeqNumber(),
		...
		ClientInterceptors.addAnInterceptor("i4", c1, conditionForInterceptingI1OnC1, conditionForExecutingI1OnC1,
				treatmentI4OnC1);
		if (LOG_ON && TEST.isInfoEnabled()) {
